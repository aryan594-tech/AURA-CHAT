...
const ChatWindow: React.FC<ChatWindowProps> = ({ contact, username }) => {
  ...
  if (!contact) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50" style={{ backgroundImage: theme.chatBackground }}>
        ...
        <h2 className="text-xl mt-4 font-semibold">Welcome to Aura Chat</h2>
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-800/50 relative" style={{ backgroundImage: theme.chatBackground, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <header className="p-4 flex items-center bg-white dark:bg-gray-900 shadow-md z-10 flex-shrink-0">
        ...
        <div className="flex items-center space-x-3 text-gray-500 dark:text-gray-400">
            <button className={`hover:${themeClasses.text} transition-colors`}><VideoCameraIcon /></button>
            <button className={`hover:${themeClasses.text} transition-colors`}><PhoneIcon /></button>
            <button onClick={() => setIsVideoCallOpen(true)} className={`hover:${themeClasses.text} transition-colors`}><ARSparklesIcon className="h-6 w-6" /></button>
        </div>
      </header>
      ...
      <main className="flex-1 p-4 overflow-y-auto custom-scrollbar">
        <div className="space-y-4">
            <p className="text-center text-xs text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-900/50 p-2 rounded-md my-2">
                <LockIcon />
                Messages are end-to-end encrypted. No one outside of this chat, not even Aura Chat, can read or listen to them.
            </p>
          {messages.map((msg) => (
