...
const AudioPlayer = ({ src, duration, isUser }: { src: string; duration?: number; isUser: boolean; }) => {
    ...
    const progressFgClass = isUser ? 'bg-white' : themeClasses.bg;
        ...
        <div className={`flex items-center space-x-2 ${playerClasses} w-60 sm:w-64`}>
            ...
            <div className="flex-grow flex flex-col justify-center">
                <div ref={progressRef} onClick={handleSeek} className={`h-1.5 ${progressBgClass} rounded-full cursor-pointer w-full`}>
                    <div className={`h-1.5 ${progressFgClass} rounded-full`} style={{ width: `${progress}%`, ...(!isUser ? themeClasses.style : {}) }}></div>
                </div>
...
const PollBubble = ({ poll, messageId, onVote, isUser }: { poll: PollDetails; messageId: string; onVote: (msgId: string, optionId: string) => void; isUser: boolean; }) => {
    ...
    const bubbleColor = isUser ? themeClasses.bg : 'bg-white dark:bg-gray-700';
    const bubbleStyle = isUser ? themeClasses.style : {};
    const textColor = isUser ? 'text-white' : 'text-gray-800 dark:text-gray-100';
    const metaColor = isUser ? 'text-gray-200/80' : 'text-gray-500 dark:text-gray-400';

    return (
        <div className={`w-72 sm:w-80 rounded-lg shadow-md ${bubbleColor}`} style={bubbleStyle}>
            <div className={`p-3 border-b ${isUser ? 'border-white/20' : 'border-gray-200 dark:border-gray-600'}`}>
            ...
            <div className="p-3 space-y-2">
                {poll.options.map(option => {
                    ...
                    return (
                        <div key={option.id}>
                            {hasVoted ? (
                                <div className="relative w-full text-sm rounded-md p-2 text-left bg-black/10 dark:bg-black/20 overflow-hidden">
                                    <div 
                                        className={`absolute top-0 left-0 h-full rounded-md ${isUser ? 'bg-white/20' : themeClasses.bg }`}
                                        style={{ width: `${percentage}%`, ...(!isUser ? themeClasses.style : { opacity: 0.3 }) }}
                                    ></div>
                                ...
                                <button
                                    ...
                                    className={`w-full text-sm rounded-md p-2 text-left transition-colors ${isUser ? `bg-white/10 ${themeClasses.hoverBg}` : 'bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500'} ${textColor}`}
                                >
                                    {option.text}
                                </button>
                            )}
...
const MessageBubble = ({ message, onVote }: { message: Message; onVote: (msgId: string, optionId: string) => void; }) => {
    ...
    const bubbleClasses = isUser
        ? `${themeClasses.bg} text-white self-end rounded-bl-none`
    ...
    const renderMessageContent = () => {
        switch (message.type) {
            case MessageType.TEXT: {
                ...
                const linkColor = isUser ? 'text-gray-200/90' : (themeClasses.customPrimaryColor ? '' : themeClasses.text);
                const linkStyle = isUser ? {} : { color: themeClasses.customPrimaryColor };
                    ...
                    <p className="text-sm whitespace-pre-wrap break-words">
                        {parts.map((part, index) => {
                             if (urlRegex.test(part)) {
                                ...
                                return <a key={index} href={href} target="_blank" rel="noopener noreferrer" className={`hover:underline ${linkColor}`} style={linkStyle}>{part}</a>;
                            }
                            return part;
                        })}
                    </p>
            ...
            case MessageType.DOCUMENT: {
                const isUserDoc = isUser ? 'text-gray-100' : 'text-gray-700 dark:text-gray-300';
                const docBg = isUser ? `bg-opacity-80 ${themeClasses.bg}` : 'bg-gray-200 dark:bg-gray-600';
                return (
                    <div className={`flex items-center space-x-3 p-3 ${docBg} rounded-lg`} style={isUser ? themeClasses.style : {}}>
            ...
            case MessageType.PAYMENT:
                 ...
                 return (
                     <div className={`w-72 rounded-lg ${isUser ? themeClasses.bg : 'bg-gray-200 dark:bg-gray-600'}`} style={isUser ? themeClasses.style : {}}>
                         <div className={`p-3 border-b ${isUser ? 'border-white/30' : 'border-gray-300 dark:border-gray-500'}`}>
                         ...
                         <div className="p-3">
                             ...
                             {note && <p className={`text-sm italic mb-3 ${isUser ? 'text-gray-100/90' : 'text-gray-600 dark:text-gray-300'}`}>"{note}"</p>}
                             <div className="flex items-center text-sm">
                                 <CheckCircleIcon className={`h-5 w-5 mr-2 ${isUser ? 'text-white/80' : 'text-green-600 dark:text-green-400'}`} />
                                 <span className={isUser ? 'text-gray-100/90' : 'text-gray-700 dark:text-gray-200'}>
                                     Transaction {message.payment.status}
                                 </span>
                             </div>
                         </div>
                         <div className={`px-3 py-2 text-xs rounded-b-lg bg-black/10 dark:bg-black/20`}>
    ...
    return (
        <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
            <div 
                className={`max-w-md md:max-w-lg shadow-sm ${isSpecialBubble ? plainBubbleClasses : `${bubbleClasses} rounded-2xl px-3 py-2`}`}
                style={isUser && !isSpecialBubble ? themeClasses.style : {}}
            >
                <div className="min-w-[50px]">
                    {renderMessageContent()}
                </div>
                 {!isSpecialBubble && (
                     <div className={`text-xs mt-1 ${isUser ? 'text-gray-200/70' : 'text-gray-400 dark:text-gray-500'} text-right`}>
                         {message.timestamp}
                     </div>
                 )}
            </div>
